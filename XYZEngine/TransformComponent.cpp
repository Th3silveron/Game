...
namespace XYZEngine
{
	...
	void TransformComponent::setWorldInfoFrom(const Matrix2D& transform) const
	{
		...
		rotation = static_cast<float>(std::atan2(transform.GetMatrix()[0][1], transform.GetMatrix()[0][0]) * 180.0 / 3.14159265);
	}
	void TransformComponent::setLocalInfoFrom(const Matrix2D& transform) const
	{
		...
		localRotation = static_cast<float>(std::atan2(transform.GetMatrix()[0][1], transform.GetMatrix()[0][0]) * 180.0 / 3.14159265);
	}
